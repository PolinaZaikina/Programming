...
namespace PhotoEnhancer
{
    internal static class Program
    {
        ...
        static void Main()
        {
            ...
            // Создание экземпляра фильтра насыщенности с использованием делегатов
            var saturationFilter = new PixelFilter<SaturationParameters>(
                "Изменение насыщенности",
                (pixel, parameters) => ChangeSaturation(pixel, parameters.Coefficient));
            mainForm.AddFilter(saturationFilter);

            mainForm.AddFilter(new TransformFilter(
                "Трансформация",
                (size) =>
                {
                    var square = Math.Min(size.Height, size.Width);
                ...
                (point, size) =>
                {
                    ...
                    {
                        double par = (double)size.Height / (double)size.Width;
                        return new Point(point.X, Convert.ToInt32((double)point.X / par));
                    }
                }
                ));
